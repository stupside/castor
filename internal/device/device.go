...
// Requirements describes what a device supports for playback.
...
// UnknownTypeError returns an error for an unrecognized device type.
...
func FindInfo(ctx context.Context, iface *net.Interface, timeout time.Duration, dtype Type, name string) (Info, error) {
	devices, err := Discover(ctx, iface, timeout)
	if err != nil {
		return Info{}, err
	}
...
func Discover(ctx context.Context, iface *net.Interface, timeout time.Duration) ([]Info, error) {
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()

	var devices []Info

	dlnaDevices, err := discoverDLNA(ctx, iface)
	if err != nil {
...
func discoverDLNA(ctx context.Context, iface *net.Interface) ([]Info, error) {
